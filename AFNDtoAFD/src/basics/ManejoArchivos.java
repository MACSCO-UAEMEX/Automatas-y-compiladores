...
*/
  ...
  /**
   * Obtienen solo el nombre del archivo de la ruta especificada
   * @param Path Ruta de donde se va a extraer el archivo
   * @return Cadena con el nombre del archivo con su extension
   */
 ...
 * @param Dir Nombre del directorio donde se van a buscar las carpetas
 * @return lista de carpetas
 */
  public String [] List_Carpetas(String Dir){
   File Ruta = new File(Dir);
 
	   if (!Ruta.exists())
	          System.out.println("ERROR: El directorio <"+ Dir +"> no existe");
	   if (!Ruta.isDirectory())
	          System.out.println("ERROR: No es un directorio <"+ Dir +">");

   return (Ruta.list(new FiltroCarpeta("")));
  }
/** Función que recibe el nombre de un directorio y la extension del archivo y 
 * regresa la lista con los nombres de los archivos que estan en esa carpeta
 * @param Dir Nombre del directorio donde se van a buscar los archivos
 * @param Ext Extension del archivo empezando con ".", ejemplo ".bin"
 * @return Lista de cadenas con los nombres de archivos que hay ahí
 */
  public String [] List_Files(String Dir, String Ext){
   File Ruta = new File(Dir);
   if (!Ruta.exists())
          System.out.println("ERROR: El directorio <"+ Dir +"> no existe");
   if (!Ruta.isDirectory())
          System.out.println("ERROR: No es un directorio <"+ Dir +">");
   return (Ruta.list(new Filtro(Ext)));  
  }
  /** Función que recibe el nombre de un directorio y la extension del archivo y
 * regresa la lista con los nombres (y su ruta absoluta) de los archivos que
 * estan en esa carpeta
 * @param Dir Nombre del directorio donde se van a buscar los archivos
 * @param Ext Extension del archivo empezando con ".", ejemplo ".bin"
 * @return Lista de cadenas con los nombres de archivos que hay ahí
 */
  public String [] List_Files_Abs(String Dir, String Ext){
   String [] Archivos = this.List_Files(Dir, Ext);

   for (int i = 0; i < Archivos.length; i++) {
       File Info = new File(this.AddToPath(Dir, Archivos[i]));
       Archivos[i] = Info.getAbsolutePath();
      }
   return (Archivos);
  }
  /**
...
/**
 * Lee un archivo de texto y regresa una arreglo con las lineas de ese archivo, pero filtra
 * las lineas que solo están en blanco (es decir que no tienen algo)
 * @param FileName Este parámetro se refiere el nombre del archivo que se desea abrir para lectura
 * @return regresa un arreglo con las lineas del archivo leido
*/
  public String [] Read_Text_File_NoNull(String FileName){
   File archivo      = null; //@var esta variable se utiliza para la descripcion del archivo
   FileReader fr     = null; //@var archivo de lectura
   FileReader fn     = null;
   BufferedReader br = null;
   BufferedReader bn = null;
     ...
     try{
       ...
       int i=0;
       while((linea=br.readLine())!=null){
         if (linea.trim().length()>0) //si tiene algo la linea se incrementa el contador
            i++;
         }//del while
...
}//de la clase Manejo de archivos
