...
public class PruebaAutomataCadena {
    ...
    public void Funcion() {
        ...
        try {
        	...
        	//Object valor_ar= recorre_map.getValue().get(0);
        
        	System.out.println("Hola soy mapa Dir:  "+aut.getDir());
        	System.out.println("Hola soy mapa EdoAct:  "+aut.getEdoact());
        	System.out.println("Hola soy mapa EdoSig:  "+aut.getEdosig());
        	System.out.println("Hola soy mapa ELido:  "+aut.getEleido());
        	System.out.println("Hola soy mapa Transi:  "+aut.getTransicion());
    ...
    public boolean ValidarCad(String cad, TreeMap<String, ArrayList> m) {
    	int indice_cinta= 0;
    	 Map<Integer, String> cinta= new HashMap<>();
         cinta.put(0, "blanco");
        String edoA = this.q;
            ...
            for (Automata auVal : aL_Au) {
                ...
                try {
                    if (sc.equals(auVal.getTransicion())) {
						...
						if (auVal.getDir() == "i") {
							indice_cinta = cinta.size() - 1;
						} else {
							indice_cinta = cinta.size() + 1;
						}
                  
                       cinta.put( indice_cinta, auVal.getEleido());
        ...
        for (String edoF : F.split(",")) {//#Validar si el estado actual al finalizar la cadena, se encuentra en un estado final y es aceptada
            if (edoA.equals(edoF.trim())) {
            	System.out.println("Hola soy Cinta:  "+ cinta);
                return 1 == 1;
            }
        }
